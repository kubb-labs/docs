...
description: Kubb 5.0 rebuilds code generation around adapters, a universal AST, parsers, and storage, and generates code up to 5.4x faster than v4.
outline: deep
...
<script setup>
const highlights = [
  { value: 5.4, prefix: 'up to ', suffix: '×', label: 'faster than v4' },
  { value: 1, label: 'shared AST, parsed once' },
  { value: 288, label: 'OpenAI operations in 3.4s' },
]
</script>
...
You feel that rewrite in three places: your config gets shorter, your generated client calls change shape, and generation runs more than five times faster. The sections below show each one. For a before and after on every breaking change, read the [migration guide](/docs/5.x/migration).

<StatHighlights :items="highlights" />

## Up to 5.4x faster generation

The [OpenAI spec](https://github.com/openai/openai-openapi) is 288 operations across 2.9 MB. Generating types, an Axios client, Zod schemas, and Faker mocks from it took 18.4 seconds in v4. v5 does the same work in 3.4 seconds.
