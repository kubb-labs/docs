...
| Spec           | Operations | v4        | v5       | Speedup   |
| -------------- | ---------- | --------- | -------- | --------- |
| `petStore.yaml`| 19         | 331.90 ms | 99.07 ms | **+235%** |
| `twitter.json` | 80         | 2,997 ms  | 711 ms   | **+322%** |
| `openai.yaml`  | 242        | 14,943 ms | 2,461 ms | **+507%** |

All three rows run `plugin-ts` with the Axios, Zod, and Faker plugins, measured on a 4-core Intel Xeon with file writing disabled so the numbers reflect the pipeline alone.
