...
- Respect `type: 'string'` when the schema carries a numeric `format` (`int64`, `uint64`, `int32`, `float`, `double`).
  ...
  A numeric format on an `integer`/`number` schema, or on a schema with no `type` at all, keeps mapping to `bigint`/`integer`/`number` as before. ([#3840](https://github.com/kubb-labs/kubb/pull/3840), [`19049fb`](https://github.com/kubb-labs/kubb/commit/19049fbf3b2e8a50e0ef4d67f224e551c4e8f702))
...
- Re-export the `Hookable` class from `@kubb/kit` (and so from the `kubb/kit` subpath), alongside the `Diagnostics`, `fsStorage`, and `memoryStorage` values already there. Code building on the kit no longer has to reach into `@kubb/core` for the hook emitter. ([#3780](https://github.com/kubb-labs/kubb/pull/3780), [`a3ed895`](https://github.com/kubb-labs/kubb/commit/a3ed895afea09174ec15f27c67607a5ea88d97fc))
...
- Keep a resolver's `file` override when a plugin resolver and the CLI load different copies of `@kubb/core`.
  ...
  `Resolver.merge` now reads a resolver's build options through a shared `Symbol.for` brand instead of `instanceof`, so the `file` override survives across `@kubb/core` copies. ([#3733](https://github.com/kubb-labs/kubb/pull/3733), [`38b5258`](https://github.com/kubb-labs/kubb/commit/38b525831eb12ca8333f7f094e1669cef37d8a78))
...
- Resolve generated file names and paths through a resolver's `file` object instead of the `resolveName` hook.
  ...
  `resolver.file`, `resolver.default.file`, and `resolver.default.path` now accept `ResolveFileOptions` / `ResolvePathOptions` instead of `(params, context)`. ([#3729](https://github.com/kubb-labs/kubb/pull/3729), [`e1bdf2d`](https://github.com/kubb-labs/kubb/commit/e1bdf2d3b9d304de03cc43634266804eee7d40a3))
...
- Merge a plugin `resolver` override per method, so a patch can change one namespace method and keep the rest.
  ...
  The `resolver` option only patches an existing resolver. Build a whole new one in a custom plugin. ([#3729](https://github.com/kubb-labs/kubb/pull/3729), [`6569340`](https://github.com/kubb-labs/kubb/commit/65693408eca54bc2b48e0642b2bed162f00c1361))
...
- Drop the renderer `stream()` capability. `Renderer.stream` is gone from `@kubb/core`'s renderer contract, `KubbDriver#dispatch` no longer looks for it, and `@kubb/renderer-jsx`'s `jsxRenderer`/`Runtime` no longer expose `stream()`.
  ...
  `Kubb.build()`'s `storage` also changed: it's the configured `Storage` backend directly rather than a view scoped to the current build's file paths. Use `files` to list what a build produced; `storage` is for reading a generated file's content back. ([#3715](https://github.com/kubb-labs/kubb/pull/3715), [`08dd244`](https://github.com/kubb-labs/kubb/commit/08dd244cbcc9b9b50244e507f78845bf8183f871))
