...
An adapter turns your input specification into the universal [AST](/docs/5.x/guide/concepts/ast). Every [plugin](/docs/5.x/guide/concepts/plugins) reads that AST, so the adapter is the only part that knows the input format.
