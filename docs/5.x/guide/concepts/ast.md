...
The `@kubb/ast` package defines Kubb's universal Abstract Syntax Tree. [Adapters](/docs/5.x/guide/concepts/adapters) produce it from a specification (OpenAPI, AsyncAPI, JSON Schema, and so on), and [plugins](/docs/5.x/guide/concepts/plugins) consume it to emit files. Because every plugin reads the same AST, one plugin works against any spec a custom adapter supplies.
...
> [!TIP]
> The AST is spec-agnostic. Plugins never look at OpenAPI directly. They read the AST the [adapter](/docs/5.x/guide/concepts/adapters) produces, which is why one plugin works for OpenAPI 2.0, 3.0, 3.1, and any custom adapter.
...
A macro is a named, composable transform built on `transform`. Macros rewrite nodes before printing, with ordering, gating, and reuse that a bare visitor does not give you. See [Concepts: Macros](/docs/5.x/guide/concepts/macros).
...
See [Concepts: Parsers](/docs/5.x/guide/concepts/parsers) for how parsers consume printers.

`defineDialect` is the adapter seam for spec-specific schema behavior. It keeps the shared converters generic, so an adapter supplies only the questions that differ between specs. See [Adapters](/docs/5.x/guide/concepts/adapters#schema-dispatch-and-dialects).
