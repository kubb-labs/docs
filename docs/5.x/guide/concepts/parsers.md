---
layout: doc
title: Parsers - Convert FileNodes to source code
description: Build Kubb parsers with defineParser. Register file extensions and turn generated FileNodes into the final string written to disk.
outline: deep
---

# Parsers

A parser turns a [`FileNode`](/docs/5.x/guide/concepts/ast) into the source string that [storage](/docs/5.x/guide/concepts/storage) writes to disk. Each parser registers the file extensions it handles, and the file processor in `@kubb/core` routes each emitted file to the matching parser.
...
Use `defineParser` from [`@kubb/core`](/docs/5.x/api/core). It is an identity wrapper that infers the parser type. It returns the object you pass in unchanged, with no per-build options:
