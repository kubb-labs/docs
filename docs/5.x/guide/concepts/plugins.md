...
Plugins teach Kubb to generate something new. A plugin owns its file naming, its output folder, its lifecycle hooks, and the [generators](/docs/5.x/api/core#generator) that walk the [AST](/docs/5.x/guide/concepts/ast) and emit `FileNode`s. Most of what you see in a generated `src/gen/` folder comes from a plugin.
...
| Diagnostics | `kubb:diagnostic`                                                        | [`KubbDiagnosticContext`](https://github.com/kubb-labs/kubb/blob/main/packages/core/src/types.ts)     | Emitted for each collected [diagnostic](/docs/5.x/api/diagnostics) during the run. |
...
A handler returns `Array<FileNode>` built with the `create*` factories from [`@kubb/ast`](/docs/5.x/guide/concepts/ast), which is the default. It may instead return JSX, in which case the generator sets `renderer: jsxRenderer` and `@kubb/renderer-jsx` walks the JSX into the same `FileNode`s. The [creating-plugins guide](/docs/5.x/guide/going-further/creating-plugins#emit-roles) names the printer, renderer, and serializer roles around that path.
