...
`transformers.name` becomes [`resolver.resolveName`](/docs/5.x/migration-guide#transformersname-resolver), and `contentType` moves to [`adapterOas`](/adapters/adapter-oas). If you wrapped a custom HTTP library through `importPath`, move that logic into your own module and call the generated functions from it.
