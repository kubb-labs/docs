...
[`@kubb/plugin-vue-query`](/docs/5.x/migration-guide/plugin-vue-query) shares these changes. On top of them, both plugins pick up the renames from `plugin-client` (`*Data`, `*Response`, `*Status<code>`).

The exported `*MutationKey` type alias is gone. Reach for the runtime helper when you need the key:
...
The change covers `queryFn`, `queryOptions`, and the hook generics in one pass. No config flag brings back the old behavior. If your client returns non-`2xx` bodies as resolved data instead of throwing, wrap it to throw so TanStack Query's `error` / `onError` path fires. The previous typing was silently broken at runtime.
...
v5 makes those parameters optional in the generated `queryKey`, `queryOptions`, and hook signatures, and the `queryFn` calls the client with a non-null assertion. The `enabled` guard stays the same.
...
You can now pass a value that is not ready yet (a route param or the result of a dependent query, say) and let the existing guard keep the query disabled until it resolves:
