...
`dateType`, `integerType`, `unknownType`, `emptySchemaType`, and `contentType` moved to [`adapterOas`](/adapters/adapter-oas/). See [Migration: @kubb/adapter-oas](/docs/5.x/migration/adapter-oas). [`resolver.name`](/docs/5.x/migration#transformersname-resolver) replaces `transformers.name` (see [Override a resolver](/docs/5.x/guide/going-further/resolvers)), and [`macros`](/docs/5.x/migration#transformersschema-macros) replace `transformers.schema`. The `generators` option is [gone](/docs/5.x/migration#generators-removed).
