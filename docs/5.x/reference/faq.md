...
Not out of the box. The default [adapter](/docs/5.x/concepts/adapters) targets OpenAPI/Swagger. For GraphQL, use [GraphQL Code Generator](https://the-guild.dev/graphql/codegen). A custom adapter can handle any format, but writing one takes real work.
...
Yes. Each plugin exposes a `resolver` option to rename operations and types. It also exposes a `macros` option to rewrite AST nodes before they are written to disk. For deeper control, write a [custom plugin](/docs/5.x/guides/creating-plugins).
...
Yes. Use `definePlugin` from `@kubb/core`. The [Creating Your First Plugin](/docs/5.x/guides/creating-plugins) guide walks through a full example.
