...
The builder must return at least `{ pluginName }`. The built-in machinery lives under `default` (`default.name`, `default.file`, `default.options`, `default.path`, `default.banner`, `default.footer`). Two top-level helpers, `name` and `file`, are injected and delegate to `default.name` and `default.file` unless the builder overrides them. Use `this` to reach sibling members, so a namespace method calls `this.name(...)` for the plugin's identifier casing.
...
| `default.file`   | Constructs a full `FileNode`; base-name casing comes from the `resolveName` param |
