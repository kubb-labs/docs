...
Behind the scenes `kubb/kit` is backed by `@kubb/kit`, which in turn re-exports from the internal `@kubb/core` and `@kubb/ast` libraries. Treat those as implementation details: always import from `kubb/kit`, never from the internal packages directly.
...
`kubb/kit` re-exports the types that go with plugin, generator, resolver, adapter, and renderer authoring. Import them alongside the functions above to type your own code. `Config` and `UserConfig`, the overall build configuration shapes, are re-exported here too.
...
The engine types come from the `kubb` package, next to `createKubb` and `defineConfig`.
...
The logger types (`Logger`, `UserLogger`, `LoggerOptions`, `LoggerContext`) and `defineLogger` live in `@kubb/cli`, needed only when building a custom CLI logger.
