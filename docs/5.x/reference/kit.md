...
The object must include at least `{ pluginName }`. The built-in machinery lives under `resolver.default` (`default.name`, `default.file`, `default.options`, `default.path`, `default.banner`, `default.footer`), exposed as a getter that always reaches the untouched defaults. Two top-level helpers, `name` and `file`, are injected and delegate to `default.name` and `default.file` unless the object overrides them. Use `this` to reach sibling members, so a namespace method calls `this.name(...)` for the plugin's identifier casing.
