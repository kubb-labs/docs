...
The builder must return at least `{ name, pluginName }`. The built-in helpers live under `core` (`core.name`, `core.fileName`, `core.options`, `core.path`, `core.file`, `core.banner`, `core.footer`) and can each be overridden.
...
export const resolver = defineResolver<MyPlugin>(() => ({
  ...
  resolveName(node) {
    return this.core.name(node.name)
  },
}))
```
...
| --------------- | ------------------------------------------------------------------------|
| `core.name`     | The plugin's generated-identifier casing                              |
| `core.fileName` | The plugin's file-name casing                                         |
| `core.options`  | Applies `exclude`, `include`, and `override` filters                   |
| `core.path`     | Resolves to `output.path`, with optional tag/path-based subdirectories |
| `core.file`     | Constructs a full `FileNode` using `core.fileName` + `core.path`       |
| `core.banner`   | Returns `output.banner` or the standard "Generated by Kubb" header     |
| `core.footer`   | Returns `output.footer` when set                                       |
