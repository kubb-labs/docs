...
| `copy`     | `(file: FileNode, source: string) => FileNode`                            | No       | By the file processor, for each `copy` file  | Turns a copied template's raw content into nodes, for example its imports as `ImportNode`s, and returns the file for `parse` to print. Omit it to write copied files verbatim. |
