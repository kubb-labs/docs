...
| `copy`     | `(file: FileNode, source: string) => string`                              | No       | By the file processor, for each `copy` file  | Adapts a copied template's raw content to the parser's output conventions, for example import extensions. `banner`/`footer` are added afterwards. Omit it to write copied files verbatim. |
