...
- `{ request?: 'zod', response?: 'zod' }` opts in per direction, where `request` validates the request body and query params before the call and `response` validates the response body after.
