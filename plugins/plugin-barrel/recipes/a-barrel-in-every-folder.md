...
A per-plugin barrel needs `output.mode: 'directory'`, since a plugin writing everything to a single file has no directories to walk. Set `output.path` too: passing your own `output` replaces the plugin's default object, so the path no longer comes along with it.
...
export default defineConfig({
  ...
  plugins: [
    pluginTs({
      output: { path: 'types', mode: 'directory', barrel: { type: 'named', nested: true } },
    }),
  ],
})
```
