...
For example, `resolver: { resolveName(name) { return \`${this.core.name(name)}Mock\` } }` appends `Mock` to every generated factory name so helpers do not clash with imported types.
