...
For example, `resolver: { name(name) { return \`${this.default.name(name)}Mock\` } }` appends `Mock` to every generated factory name so helpers do not clash with imported types.
