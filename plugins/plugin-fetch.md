...
The runtime sets `method`, `headers`, `body`, `signal`, and `credentials` on each `fetch` itself. To reach the rest of the `RequestInit` (`cache`, `mode`, `redirect`, `keepalive`, `duplex`, or Next.js's `next: { revalidate, tags }`), pass `fetchOptions`. It works at the client level for every call and per request, where a per-request value wins:
...
The transport spreads `fetchOptions` into the `fetch` init before the fields the runtime owns, so it can never override the method, headers, body, signal, or credentials.
...
This plugin needs `@kubb/plugin-ts` in your config. Kubb runs it before `plugin-fetch` so the functions can import the generated types. When `parser` is set to `'zod'`, also add `@kubb/plugin-zod`.
