...
Without the override, that same field prints `id: z.bigint().optional().meta({ examples: [10] })`. The `integer` node (for 32-bit `format: int32` fields which emit `z.int32()`) is a separate handler from `bigint`, so overriding one does not affect the other.
